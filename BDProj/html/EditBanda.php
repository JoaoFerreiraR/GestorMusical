  ...
  <body>
      ...
      if($pass == $CheckPass){ //se user for legit
      	if($func=='0') header('Location: http://alunos.deec.uc.pt/~jferreira/BDProj/html/dashboard.php');
      		if($func=='1'and strtolower($user) !=strtolower($_GET["Banda"])) header('Location: http://alunos.deec.uc.pt/~jferreira/BDProj/html/dashboard.php');
        ?>
...
<?php
              $BandNAME= $_GET["Banda"];
              $SQLBANDA= "SELECT Banda.NomeBanda,DataInicio,DataFim,imagem,PaisOrigem from Banda  where Banda.NomeBanda = '$BandNAME';";

              $GetBand=mysqli_query($ligacao,$SQLBANDA);
               if (mysqli_num_rows($GetBand)==0) { //nao existe esse user
               $URL="http://alunos.deec.uc.pt/~jferreira/BDProj/html/dashboard.php";
               echo "<script type='text/javascript'>document.location.href='{$URL}';</script>";
               echo '<META HTTP-EQUIV="refresh" content="0;URL=' . $URL . '">';
              }
              while($tuplo = mysqli_fetch_array($GetBand)) {
               $NomeBanda=$tuplo["NomeBanda"];
              $DataInicio=$tuplo["DataInicio"];
              $PaisOrigem=$tuplo["PaisOrigem"];
               $DataFim= $tuplo["DataFim"];
               if($DataFim==NULL) $DataFim = "N/A";
       	...
       	if(!empty($_POST["Oday"])) { //se tivermos posto data de fim
       		if(date($NovaDataN)>date($NovaDataO)){  
    			echo "A Data de Fim tem de ser maior que a de Fundação.";
                   	...
                   	$SQL1="UPDATE Banda
                        SET DataInicio='$NovaDataN',DataFim ='$NovaDataO',imagem='$NomeFicheiro'
                        Where NomeBanda ='$BandNAME';
                        ";
                   	$rez=mysqli_query($ligacao,$SQL1);
                   	//inserir Musicos na banda
                    $limparMusicos= "DELETE from FazParte where NomeBanda='$BandNAME';";
                    $clear=mysqli_query($ligacao,$limparMusicos);
                     $N = count($ArtistasIDs);
                      for($i=0; $i < $N; $i++) {
                          $SQL3= "INSERT into FazParte(IDArtista,NomeBanda) values ('$ArtistasIDs[$i]','$BandNAME');";
                         $rez3=mysqli_query($ligacao,$SQL3);
                        }
                        
                        if($rez and $rez3) echo "</div><div class='bg-success text-white text-center'>Foi editada a Banda com sucesso!";
          }
       	}else{
       		
       		         $SQL1="UPDATE Banda
                          SET DataInicio='$NovaDataN',imagem='$NomeFicheiro'
                          Where NomeBanda ='$BandNAME';";
                    $rez=mysqli_query($ligacao,$SQL1);
                    $limparMusicos= "DELETE from FazParte where NomeBanda='$BandNAME';";
                    $clear=mysqli_query($ligacao,$limparMusicos);
...
<div class="row">
 ...
 <div class="card mb-3">
      <div class="card-header">
        <i class="fa fa-user-plus"></i> Editar Banda</div>
          ...
          <div class="card-body">
            <div class="form-group">
             ...
             <label for="bday">Data de Fundação</label>
                 <input type="date" name="Bday" value="<?php echo $DataInicio;?>" required >

             <label for="bday">Data de Fim</label>
                 <input type="date" name="Oday" value="<?php echo $DataFim;?>" >
                </div>
                </div>
...
<option value="<?php echo $PaisOrigem;?>" selected><?php echo $PaisOrigem;?></option> 
...
<option value="Zimbabwe">Zimbabwe</option>
            ...
            <div class="card-footer small text-right">
              <button type="submit" name = "NovaBandaSub" class="btn btn-success btn-lg" title ="Adicionar Banda"><i class="fa fa-pencil"></i> Editar Banda</button>
            </div>
          </div>
        </div>
      </form>
