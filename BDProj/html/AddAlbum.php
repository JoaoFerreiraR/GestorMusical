...
// if everything is ok, try to upload file
    ...
    if (move_uploaded_file($_FILES["file"]["tmp_name"], 'images/albums/'.$NomeFicheiro)) {
        ...
        $EstiloNovo = $_POST["Estilo"];
                   ...
                   $SQL1="INSERT into Album(Nome,ano,estilo,foto,NomeBanda,IDEditora,review) values('$NovoAlbumNome','$AnoNovo','$EstiloNovo','$NomeFicheiro','$NovaBandaNome','$EditoraNova',NULL);";
                    $rez=mysqli_query($ligacao,$SQL1);
                        if($rez) echo "</div><div class='bg-success text-white text-center'>Foi criado um novo Álbum com sucesso! <br>Para lhe adicionar Músicas carregue <a href='AddMusicas.php'>Aqui</a>";
...
<div class="row">
 ...
 <div class="card mb-3">
      <div class="card-header">
        <i class="fa fa-user-plus"></i> Adicionar Álbum</div>
        <form action = "" method = "post"  enctype="multipart/form-data">
          <div class="card-body">
              ...
              <input type="file" class="form-control-file" id="file" aria-describedby="fileHelp" name= "file" id= "file" required>
               <small id="fileHelp" class="form-text text-muted">A imagem do Álbum tem de ser em formato JPG e ter menos de 1Mb.</small>
                 <label for="bday">Estilo</label>
                  <input class="form-control" id="exampleInputName" name = "Estilo" type="text" aria-describedby="nameHelp" placeholder="Introduza o estilo de Música" required>
        </div>
                <div class="col-md-6 ">
                  <div class="col ">
                  <label for="exampleInputName">Nome do Álbum</label>
                  <input class="form-control" id="exampleInputName" name = "NomeAlbum" type="text" aria-describedby="nameHelp" placeholder="Introduza o Nome do Álbum" required>
                  </div>
                    <div class="col">
            ...
            <div class="card-footer small text-right">
              <button type="submit" name = "NovoAlbumSub" class="btn btn-success btn-lg" title ="Adicionar Album"><i class="fa fa-plus"></i> Adicionar Álbum</button>
            </div>
          </div>
        </div>
      </form>
